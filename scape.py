...
# written messages
nav_link = 'https://www.presidency.ucsb.edu/documents/app-categories/presidential/written-messages?items_per_page=60'
...
while has_next_nav:
    ...
    for link in page_links:
        ...
        pres_name = pres_name_raw.text.strip()

        byline_raw = page_soup.find(class_='diet-by-line president')
        byline = byline_raw.text.strip()
        ...
        date = re.sub('(\d+-\d+-\d+).*', '\g<1>', date_raw).strip()
        ...
        for paragraph in paragraphs_raw:
            ...
            if re.match('\[.*\]', content):
                garbage_collected.append(paragraph.text)
                continue

            if len(paragraph.contents) > 1:
                contents = [c.text for c in paragraph.contents]
                contents_merged = ' '.join(contents)
                content = re.sub('\s{2,}', ' ', contents_merged)
